...
/**
 * This is a balanced tree implementation of a LongMap. There are existing two main implementation
 * of mapping in Java:
 *    1. based on using trees
 *    2. based on using arrays and linked-lists combined.
 * This implementation of LongMap based on triple-linked list known as Red-Black Tree. This
 * implementation is similar to the implementation of a tree-map, but adapted for a specific task
 * (storage of longs as keys) and does not use comparators, preferring natural ordering.
 * Therefore, the method used in this class is the most successful in terms of efficiency and memory
 * usage.
 */
public class LongMapOnBalancedTree<V> implements LongMap<V> {
  ...
  private V supplementTree(long key, V value) {
    ...
    makeCorrectTreeAfterInsertion(entry);
    this.size++;
    return value;
  }
  ...
  private void deleteEntry(Entry<V> entry) {
    ...
    if (replacement != null) {
      ...
      if (entry.isBlack) {
        makeCorrectTreeAfterDeletion(replacement);
      }
    } else if (entry.parent == null) {
      rootEntry = null;
    } else {
      if (entry.isBlack) {
        makeCorrectTreeAfterDeletion(entry);
      }
      if (entry.parent != null) {
  ...
  private void makeCorrectTreeAfterInsertion(Entry<V> entry) {
    entry.isBlack = false;
  ...
  private void makeCorrectTreeAfterDeletion(Entry<V> entry) {
  ...
  private void collectValues(Entry<V> entry, List<V> values) {
    if (entry != null) {
      values.add(entry.value);
      if (entry.leftChild != null) {
        collectValues(entry.leftChild, values);
      }
      if (entry.rightChild != null) {
        collectValues(entry.rightChild, values);
      }
    }
  }
